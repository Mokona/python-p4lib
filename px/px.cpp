...
/* 'px' console launch executable.  
 * 
 * This program exists solely to launch "python <installdir>/px.py <argv>" on
 * Windows. "px.py" must be in the same directory.
 *
 * Rationale:
 *    - On some Windows flavours .py *can* be put on the PATHEXT to be
 *      able to find "px.py" if it is on the PATH. This is fine until
 *      you need shell redirection to work. It does NOT for extensions
 *      to PATHEXT.  Redirection *does* work for "python <script>.py" so
...
int main(int argc, char** argv)
{
    ...
    // determine the full path to "px.py"
    char pxPy[MAXPATHLEN+1];
    snprintf(pxPy, MAXPATHLEN, "%s%cpx.py", programPath, SEP);
    
    // build the argument array for launching px.py
    char* pythonArgs[MAX_PYTHON_ARGS+1];
    int nPythonArgs = 0;
    pythonArgs[nPythonArgs++] = "python";
    pythonArgs[nPythonArgs++] = "-tt";
    pythonArgs[nPythonArgs++] = pxPy;
