...
class FlushTestCase(unittest.TestCase):
    ...
    def test_flush_added_files(self):
        top = os.getcwd()
        andrew = testsupport.users['andrew']
        bertha = testsupport.users['bertha']
        p4 = P4()
        fnames = ['test_flush_added_files_1.txt',
                  'test_flush_added_files_2.txt']
        ...
        try:
            ...
            for fname in fnames:
                ...
                fout.close()
            p4.add(fnames)
            p4.submit(fnames, 'for test_flush_added_files')
        finally:
            os.chdir(top)
        ...
        try:
            ...
            results = p4.flush(fnames)
            self.failUnless(len(results) == len(fnames))
        finally:
            os.chdir(top)

    def test_flush_file_needing_resolve(self):
        top = os.getcwd()
        andrew = testsupport.users['andrew']
        bertha = testsupport.users['bertha']
        p4 = P4()
        fname = 'test_flush_file_needing_resolve.txt'
...
def suite():
    ...
    return unittest.makeSuite(FlushTestCase)
